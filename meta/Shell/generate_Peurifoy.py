...
def simulate(radii):#, lamLimit=400, orderLimit=None, epsIn=None):
    # Moved from parameter default value to here due to parallelism
    lamLimit=400
    orderLimit=None
    epsIn=None
    ...
    processed_spect = np.reshape(processed_spect, [1, -1])
    # Saving for the multiprocessing
    # if Ypred_mat is not None:
    #     print('shape of Ypred save', np.shape(Ypred_mat))
    #     print('shape of processed spec', np.shape(processed_spect))
    #     # Ypred_mat[ind:ind+1, :] = processed_spect
    #     Ypred_mat[ind] = 1
    #     print('type of secptra',type(processed_spect))
    #     print('saving to Ypresave')
    #     print(processed_spect)
    #     print(Ypred_mat[ind, :])

    return processed_spect

def multi_process_simulate(Xpred, num_cpu = 10):
    # Ypred_save = np.zeros([len(Xpred), 201])
    # The multiprocessing step for the Chen simuation
    try: 
        pool = Pool(num_cpu)
        args_list = []
        for ind in range(len(Xpred)):
            args_list.append((Xpred[ind],))
        Ypred_list = pool.starmap(simulate, args_list)
        # print(args_list)
        # print('checking in collecting, they are not all zero')
        # print(Ypred_save[0, :])
        Ypred = np.array(Ypred_list)
        Ypred = np.reshape(Ypred, [-1, 201])
        assert Ypred[0][0] != 0, 'Your Ypred save is all zero again'
        return Ypred
    finally:
        pool.close()
        pool.join()
...
if __name__ == '__main__':
    ...
    try: 
        ...
        for i in range(num_cpu):
            args_list.append((i, 5000//num_cpu))
        # print((args_list))
        # print(len(args_list))
        pool.starmap(generate_multi_processing, args_list)
    finally:
        pool.close()
        pool.join()
