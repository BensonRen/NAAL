...
def simulate(Xpred):
    ...
    # if Ypred_save is not None:
    #     Ypred_save[start:end, :] = Ypred
    #     print('in Chen simulator')
    #     print('type of Ypred :')
    #     print(type(Ypred))
    #     print(np.shape(Ypred))
    # return np.ravel(Ypred)
def simuate_single(x, scaled_cheb, map2, n_spct):
        # nonlocal scaled_cheb, map2, n_spct
        dataY = np.zeros((n_spct, 3))
...
def multi_process_simulate(Xpred, num_cpu = 5):
    ...
    try: 
        ...
        for i in range(len(Xpred)):
            args_list.append((Xpred[i, :], scaled_cheb, map2, n_spct))
        # i = 0
        # chunk = len(Xpred) // num_cpu
        ...
        #     else:
        #         end = (i+1) * chunk
        #     args_list.append((np.copy(Xpred[start:end]), ))
        # print(args_list)
        Ypred_list = pool.starmap(simuate_single, args_list)
        # for i in range(len(Ypred_list)):
        #     print('shape of {}-th {}'.format(i, np.shape(Ypred_list[i])))
        Ypred = np.concatenate(Ypred_list)
        Ypred = np.reshape(Ypred, [-1, 256])
        # print('shape of stacked array', np.shape(Ypred))
        assert np.shape(Ypred)[1] == 256, 'The stacked array shape is wrong'
        # Ypred = np.reshape(Ypred, [-1, 256])
        return Ypred
    finally:
        pool.close()
